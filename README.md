...
Проект предоставляет полный цикл развёртывания и тестирования отказоустойчивости Strimzi Kafka в Kubernetes: мониторинг (VictoriaMetrics K8s Stack, Grafana), сбор логов (VictoriaLogs, victoria-logs-collector), верификация доставки через Redis и хаос-эксперименты (Chaos Mesh). Все операции с кластером выполняются через **kubectl** при наличии доступа к k8s.
