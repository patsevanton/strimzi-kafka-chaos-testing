...
1. **Два хранилища без транзакции** — при сбое между записью в Kafka и Redis теряется консистентность: «нет в Redis» может означать «ещё не записали» или «уже обработали».
2. **Порядок Kafka → Redis** — при падении между ними сообщение есть в Kafka, но нет в Redis. Обратный порядок создаёт «сироты» в Redis. Текущий порядок предпочтительнее.
3. **Нет идемпотентности** — при at-least-once повторная доставка удаляет ключ из Redis при первой обработке; вторая обработка неотличима от отсутствия записи. Счётчик `received` завышается.
4. **SLO по old-ключам** — не ловит рост задержки, если все сообщения обрабатываются быстрее порога N секунд. Нужно смотреть и pending, и old.
5. **Коллизия ключей** — при нескольких репликах Producer `key-{messageID}` может пересекаться. Нужен UUID или namespace+id.
6. **Утечка памяти Redis** — без TTL ключи накапливаются при остановке Consumer. Рекомендуется TTL (например, 24 ч) для очистки.
7. **Нагрузка на Redis** — 2 операции на Producer, 3 на Consumer + периодический SCAN. При высокой частоте Redis становится узким местом.

**Итог:** схема полезна для верификации целостности и оценки SLO в тестовых сценариях. Для продакшена нужны: уникальные ключи, TTL, обработка недоступности Redis, дедупликация.
