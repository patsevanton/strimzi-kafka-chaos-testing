...
Реализованная схема: Producer пишет в Kafka и в Redis (ключ = ключ сообщения, значение = хеш тела + timestamp). Consumer сверяет хеш полученного тела с Redis, при совпадении удаляет ключ из Redis и увеличивает счётчик полученных сообщений. SLO считается по доле сообщений, «оставшихся» в Redis дольше заданного времени.
